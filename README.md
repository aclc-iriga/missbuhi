...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/missbuhi`.
...
5. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `missbuhi` and import [missbuhi.sql](missbuhi.sql) into it.

6. Open the terminal and navigate to the project directory **missbuhi**.
...
9. Open your web browser and access <http://localhost:5198/missbuhi/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-missbuhi` and import [missbuhi.sql](missbuhi.sql) into it.
...
2. Access the application by visiting `http://[host_name]/missbuhi`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/missbuhi>
     - <http://192.168.1.99/missbuhi>


#### Admin Dashboard
Access the **admin dashboard** at <http://localhost/missbuhi/app>.
